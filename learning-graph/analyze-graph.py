...
def find_terminal_nodes(concepts: Dict[int, str],
    ...
    """Find terminal nodes: concepts that nothing depends on (outdegree=0, potential dead ends)."""
...
def generate_report(csv_path: str, output_path: str):
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        if len(terminal) > 50:
            f.write(f"- ⚠️ **Many terminal nodes** ({len(terminal)}): Consider if these should be prerequisites for advanced concepts\n")
        if is_dag:
