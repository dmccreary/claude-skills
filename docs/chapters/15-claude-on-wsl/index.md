...
It is often difficult to get the Windows Systems for Linux (WSL) working on Windows
due to the high variability of support of virtual machines on Windows hardware.
You must make sure your computer support the Hyper-V options.
On older Windows PCs this could only be set in the BIOS during boot.
You can install Claude Desktop on the PowerShell and then have Claude
